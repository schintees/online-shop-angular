declare namespace Cypress {
    interface Chainable<Subject = any> {
        ...
        login(username: string, password: string, shouldFail?: boolean): typeof login;
...
function clickOnIcon(iconType: string): void {
    cy.get('mat-icon').filter(`:contains(${iconType})`).parent().click();
}

function shouldOpenSnackBar(isError: boolean = false): void {
    cy.get('mat-snack-bar-container').should('be.visible').should('have.class', isError ? 'app-notification-error' : 'app-notification-success');
}

function login(username: string, password: string, shouldFail: boolean = false): void {
    cy.get('[formcontrolname="username"]').type(username);
    cy.get('[formcontrolname="password"]').type(password);

    cy.get('button[type=submit]').click().should(() => {
        shouldFail ? expect(localStorage.getItem('token')).to.be.null : expect(localStorage.getItem('token')).to.not.be.null;
    });
}

function tableContainsElements(flag: boolean = true): void {
    cy.get('tbody').find('tr').should(flag ? 'exist' : 'not.exist');
}
...
Cypress.Commands.add('clickOnFirstTableRowtButton', clickOnFirstTableRowtButton)
